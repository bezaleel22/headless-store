...
export const paystackHandler = new PaymentMethodHandler({
    code: 'paystack-payment-handler',
    description: [
        {
            languageCode: LanguageCode.en,
            value: 'Paystack payment gateway',
        },
    ],
    args: {
        apiKey: {
    ...
    createRefund: async (
        ...
        Logger.warn(
            `This plugin does not support refunds. You need to manually refund ${payment.transactionId} via Paystack`,
            loggerCtx
        );
        return {
            state: 'Failed',
            metadata: {
                public: {
                    message: `This plugin does not support refunds. You need to manually refund ${payment.transactionId} via Paystack`,
                },
            },
        };
    },
